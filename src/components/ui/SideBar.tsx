            ...
            if (item.key === "dashboard" && item.type === "dropdown") {
              return {
                ...
                children: item.children.filter(
                  (child) => !["message", "program", "newsletter"].includes(child.key)
                ),
              };
            }
            return item;
      ...
      >
        ...
        <div className="p-4 border-t border-slate-100/50 bg-white/30 backdrop-blur-sm">
          <div className="relative">
            ...
            {/* User Card */}
            <div className={`flex items-center gap-3 p-3 rounded-2xl transition-all duration-300 border cursor-pointer ${open ? 'bg-white border-violet-100 shadow-lg shadow-violet-100/50' : 'bg-white/60 border-white/60 hover:bg-white hover:shadow-sm'}`} onClick={() => setOpen(!open)}>
              ...
              <button
                className={`p-1.5 rounded-xl transition-all ${open ? "bg-violet-50 text-violet-600" : "text-slate-400 hover:bg-slate-50"}`}
              >
