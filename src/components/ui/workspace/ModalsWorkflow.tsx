...
import {
  ...
  Copy, Save as SaveIcon, File as FileIcon, Download
} from "lucide-react";
...
export default function ModalsWorkflow({ isOpen, onClose, task, onTaskUpdated }: { isOpen: boolean; onClose: () => void; task?: any; onTaskUpdated?: () => void }) {
  ...
  useEffect(() => {
    const load = async () => {
          ...
          try {
            ...
            if (labels.length > 0) {
              currentLabels = labels.map(L => ({
                id: L.id, name: L.name, color: L.color, bgColor: L.bgColor, textColor: L.textColor
              }));
            } else { currentLabels = [...INITIAL_TAGS]; }
          ...
          try {
            ...
            if (checklistItems.length > 0) {
              ...
              const checklistBlock: ContentBlock = {
                id: blockId, type: 'checklist', title: 'Checklist',
      ...
      } catch (err) { console.error("Failed to load task", err); } finally { setLoadingTask(false); }
    };
    load();
    return () => { if (!isOpen) setBlocks([]); };
  }, [isOpen, task?.id]);
      ...
      try {
        ...
        if (type === "checklist:created") {
          ...
          setBlocks(prev => {
            ...
            if (idx === -1) return [{ id: `checklist-${Date.now()}`, type: 'checklist', title: 'Checklist', items: [{ id: item.id, text: item.text, isChecked: item.isChecked }] }, ...prev];
  ...
  useEffect(() => {
    if (!isOpen) { setMoreOpen(false); setDropdownStyle(null); }
  }, [isOpen]);
  ...
  useEffect(() => {
    const handler = (e: MouseEvent) => {
      ...
      if (!insideMenu && !insideDropdown) { setMoreOpen(false); setDropdownStyle(null); }
    };
    document.addEventListener('mousedown', handler);
    return () => document.removeEventListener('mousedown', handler);
  }, [menuRef, dropdownRef]);
  ...
  const toggleMore = () => {
    ...
    if (open && menuRef.current) {
      ...
      const width = 176; let left = rect.right - width; if (left < 8) left = rect.left;
  ...
  const logActivity = (action: string) => {
    setActivities(prev => [{ id: Date.now().toString(), user: "You", action, time: new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }) }, ...prev]);
    if (boardId) createActivity({ boardId, user: "You", action, target: title, projectId: boardId, taskId: taskId || undefined }).catch(e => console.error("createActivity failed", e));
  };
  ...
  const handleArchiveTask = async () => {
    ...
    try {
      setIsSaving(true);
      let cols: any[] = []; try { cols = await getColumns(boardId); } catch (e) { cols = []; }
      let archived = cols.find(c => (c.name || '').toLowerCase() === 'archived');
      if (!archived) archived = await createColumn({ boardId, title: 'Archived' });
      ...
      onClose();
    } catch (err) { console.error('Failed to archive task', err); alert('Failed to archive task'); } finally { setIsSaving(false); setMoreOpen(false); }
  };
  ...
  const handleDeleteTask = async () => {
    ...
    try {
      ...
      try { onTaskUpdated?.(); } catch (e) { /* ignore */ }
      onClose();
    } catch (err) { console.error('Failed to delete task', err); alert('Failed to delete task'); } finally { setIsSaving(false); setMoreOpen(false); }
  };
  ...
  const handleCreateTag = async () => {
    ...
    try {
      const created = await createLabel({ boardId, name: newTagName, color: newTagColor.name, bgColor: newTagColor.labelBg, textColor: newTagColor.labelText });
      const newTag: TagItem = { id: created.id, name: created.name, color: created.color, bgColor: created.bgColor, textColor: created.textColor };
      setAvailableTags(prev => [...prev, newTag]); setSelectedTagIds(prev => [...prev, newTag.id]);
      setTagView('list'); setNewTagName(""); logActivity(`created label "${newTagName}"`);
    } catch (err) { console.error("Failed to create tag", err); }
  };

  const isValidObjectId = (id: string | null | undefined) => { if (!id || typeof id !== 'string') return false; return /^[a-fA-F0-9]{24}$/.test(id); };
    ...
    if (editingTagId && newTagName.trim()) {
      try {
        if (!isValidObjectId(editingTagId)) {
          ...
          const created = await createLabel({ boardId, name: newTagName, color: newTagColor.name, bgColor: newTagColor.labelBg, textColor: newTagColor.labelText });
          const createdTag: TagItem = { id: created.id, name: created.name, color: created.color, bgColor: created.bgColor, textColor: created.textColor };
          ...
          setTagView('list'); setEditingTagId(null); setNewTagName(""); logActivity(`created label "${newTagName}"`);
          return;
        }
        const updated = await updateLabel(editingTagId, { name: newTagName, color: newTagColor.name, bgColor: newTagColor.labelBg, textColor: newTagColor.labelText });
        const updatedTag: TagItem = { id: updated.id, name: updated.name, color: updated.color, bgColor: updated.bgColor, textColor: updated.textColor };
        setAvailableTags(prev => prev.map(t => t.id === editingTagId ? updatedTag : t));
        setTagView('list'); setEditingTagId(null); setNewTagName(""); logActivity(`updated label "${newTagName}"`);
  ...
  const handleDeleteTag = async () => {
    if (editingTagId) {
      ...
      try {
        ...
        if (!isValidObjectId(editingTagId)) {
          setAvailableTags(prev => prev.filter(t => t.id !== editingTagId)); setSelectedTagIds(prev => prev.filter(id => id !== editingTagId));
          setTagView('list'); setEditingTagId(null); setNewTagName(""); logActivity(`deleted label "${tagToDelete?.name}"`);
          return;
        }
  ...
  const startEditTag = (tag: TagItem) => { setEditingTagId(tag.id); setNewTagName(tag.name); setNewTagColor(TAG_COLORS.find(c => c.name === tag.color) || TAG_COLORS[0]); setTagView('edit'); };
  const startCreateTag = () => { setEditingTagId(null); setNewTagName(""); setNewTagColor(TAG_COLORS[0]); setTagView('create'); };
  const toggleTag = (tagId: string) => {
    const tag = availableTags.find(t => t.id === tagId); if (!tag) return;
    (async () => {
      if (!taskId) { if (selectedTagIds.includes(tagId)) setSelectedTagIds(prev => prev.filter(id => id !== tagId)); else setSelectedTagIds([tagId]); return; }
      if (selectedTagIds.includes(tagId)) {
        setSelectedTagIds(prev => prev.filter(id => id !== tagId));
        try { await updateTask(String(taskId), { tag: undefined, tagColor: undefined }); logActivity(`removed label "${tag.name}"`); } catch (err) { console.error('Failed to remove tag from task', err); }
      } else {
        setSelectedTagIds([tagId]);
        try { await updateTask(String(taskId), { tag: tag.name, tagColor: tag.color }); logActivity(`added label "${tag.name}"`); } catch (err) { console.error('Failed to set tag on task', err); }
      }
    })();
  };
  ...
  const addChecklistBlock = async () => {
    ...
    logActivity(`added checklist "${titleText}"`); setNewChecklistTitle(""); setActivePopover(null);
    if (taskId) { try { const newCount = checklistCount + 1; setChecklistCount(newCount); await updateTask(String(taskId), { checklist: newCount }); } catch (err) { console.error("Failed to update checklist count", err); } }
  };
  const addChecklistItem = async (blockId: string, text: string) => {
    ...
    } catch (err) { console.error("Failed to create checklist item", err); setBlocks(prev => prev.map(b => b.id === blockId ? { ...b, items: b.items?.filter(i => i.id !== tempId) } : b)); }
  };
  const toggleCheckItem = async (blockId: string, itemId: string) => {
    ...
    setBlocks(prev => prev.map(b => { if (b.id === blockId) { const updated = b.items?.map(i => i.id === itemId ? { ...i, isChecked: !i.isChecked } : i); return { ...b, items: updated }; } return b; }));
    try {
    ...
    } catch (err) { console.error("Failed to toggle check item", err); setBlocks(prev => prev.map(b => b.id === blockId ? { ...b, items: b.items?.map(i => i.id === itemId ? { ...i, isChecked: currentItem?.isChecked ?? false } : i) } : b)); }
  };
  const deleteCheckItem = async (blockId: string, itemId: string) => {
    ...
    try { if (!itemId.startsWith('temp-')) { await deleteChecklistItem(itemId); logActivity("deleted checklist item"); } } catch (err) { console.error("Failed to delete check item", err); }
  };
  const updateCheckItem = async (blockId: string, itemId: string, text: string) => {
    ...
    try { if (!itemId.startsWith('temp-')) { await updateChecklistItem(itemId, { text }); logActivity(`updated checklist item to "${text}"`); } } catch (err) { console.error("Failed to update check item", err); }
  };
  const deleteBlock = async (blockId: string) => {
    if (confirm("Delete this section?")) {
      const block = blocks.find(b => b.id === blockId);
      setBlocks(prev => prev.filter(b => b.id !== blockId)); logActivity("removed a block");
      if (block?.type === 'checklist' && taskId) { try { const newCount = Math.max(0, checklistCount - 1); setChecklistCount(newCount); await updateTask(String(taskId), { checklist: newCount }); } catch (err) { console.error("Failed to update checklist count", err); } }
    }
  };
  ...
  const handleAddAttachment = async (type: 'link' | 'file', val1: string, val2?: string) => {
    ...
    if (taskId) { try { await updateTask(String(taskId), { attachments: (task?.attachments || 0) + 1 }); } catch (err) { console.error("Failed to update attachments count", err); } }
  };
  ...
  const submitComment = async (content: string) => {
    ...
    } catch (err) { console.error('Failed to persist comment', err); setComments(prev => prev.filter(c => !c.id.startsWith('temp-'))); }
  };
  ...
  const deleteComment = async (id: string) => {
    ...
    try { if (!id.startsWith('temp-')) { await apiDeleteComment(id); logActivity('deleted comment'); } } catch (err) { console.error('Failed to delete comment', err); }
  };
  ...
  const saveField = async (data: { title?: string; description?: string }) => {
    if (!taskId) return;
    try { await updateTask(String(taskId), data); if (data.title) logActivity("updated title"); if (data.description) logActivity("updated description"); } catch (err) { console.error("Failed to save field", err); }
  };
  ...
  const handleSaveAll = async () => {
    ...
    try {
      ...
      const payload: any = { title, description: desc, assigneeIds: assignedMembers, dueDate: due, checklist: checklistCount };
      await updateTask(String(taskId), payload);
      logActivity('saved card');
      try { onTaskUpdated?.(); } catch (e) { /* ignore */ }
    } catch (err) { console.error('Failed to save card', err); } finally { setIsSaving(false); }
  };
        ...
        <div className="bg-white w-full max-w-6xl h-[90vh] rounded-[20px] shadow-2xl overflow-hidden flex flex-col ring-1 ring-slate-200 font-sans relative">
          ...
          <div className="bg-white px-6 py-4 flex items-center justify-between border-b border-slate-200 shrink-0 shadow-sm z-10">
            <div className="flex items-center gap-3">
              <button onClick={() => setIsAccepted(!isAccepted)} className={`flex items-center gap-2 px-3 py-1.5 rounded-lg text-sm font-bold transition-all shadow-sm active:scale-95 border ${isAccepted ? 'bg-green-100 text-green-700 border-green-200' : 'bg-white text-slate-700 border-slate-200 hover:bg-slate-50'}`}>
            ...
            <div className="flex gap-2 items-center">
              ...
              <div className="relative" ref={menuRef}><button onClick={toggleMore} className="p-2 rounded-lg text-slate-500 hover:bg-slate-100"><MoreHorizontal size={20} /></button></div>
              {moreOpen && dropdownStyle && portalNodeRef.current && createPortal(<div ref={dropdownRef} style={dropdownStyle} className="bg-white rounded-md border shadow-lg z-9999"><button onClick={handleArchiveTask} className="w-full text-left px-4 py-2 hover:bg-slate-50 flex items-center gap-2 text-sm text-slate-700"><Archive size={16} /> Archive task</button><button onClick={handleDeleteTask} className="w-full text-left px-4 py-2 hover:bg-red-50 text-red-600 flex items-center gap-2 text-sm"><Trash2 size={16} /> Delete task</button></div>, portalNodeRef.current)}
              ...
              <div className="mb-6 space-y-4">
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1">
                    <input value={title} onChange={e => setTitle(e.target.value)} onBlur={() => saveField({ title })} className="w-full text-3xl font-black text-slate-900 bg-transparent outline-none placeholder:text-slate-300" />
              ...
              <div className="ml-9 mb-8 flex flex-wrap gap-4">
                ...
                {dateRange?.from && (
                  <div className="flex items-center gap-2 bg-slate-100 px-3 py-1.5 rounded-md text-sm font-bold text-slate-700 cursor-pointer hover:bg-slate-200 transition-colors" onClick={() => setActivePopover('dates')}>
                    <Clock size={16} />
                    <span>{format(dateRange.from, 'dd MMM')}{dateRange.to ? ` - ${format(dateRange.to, 'dd MMM')}` : ''}</span>
                  </div>
                )}
              </div>
              <div className="mb-8 group">
                <div className="flex items-center gap-3 mb-2 text-slate-800 font-bold"><AlignLeft size={20} /><h3>รายละเอียด</h3></div>
                <div className="ml-9">
                  <textarea value={desc} onChange={(e) => setDesc(e.target.value)} onBlur={() => saveField({ description: desc })} placeholder="เพิ่มรายละเอียดเพิ่มเติมเกี่ยวกับงานนี้..." className="w-full min-h-[100px] bg-slate-50 hover:bg-slate-100 focus:bg-white border border-transparent focus:border-blue-500 rounded-lg p-4 text-base text-slate-900 leading-relaxed transition-all resize-none focus:ring-2 focus:ring-blue-100" />
                </div>
              </div>
                    ...
                    {block.type === 'checklist' && (
                      <div className="ml-9 space-y-4">
                        <div className="flex items-center gap-3 text-xs text-slate-500 font-bold mb-2">
                          ...
                          <div className="flex-1 h-2 bg-slate-200 rounded-full overflow-hidden"><div className="h-full bg-emerald-500 transition-all duration-300" style={{ width: `${((block.items?.filter(i => i.isChecked).length || 0) / (block.items?.length || 1)) * 100}%` }}></div></div>
                        </div>
                        ...
                        <div className="pl-7 mt-2">
                          <button className="text-sm text-slate-500 hover:text-slate-800 hover:bg-slate-100 px-3 py-1.5 rounded-md transition-colors flex items-center gap-2 mb-2" onClick={() => { const input = document.getElementById(`add-input-${block.id}`) as HTMLInputElement; if (input) input.focus(); }}><Plus size={16} /> Add an item</button>
                      ...
                      <div className="ml-9 grid grid-cols-1 gap-2">
                        {block.attachments?.map(att => (
                          <div key={att.id} className="flex items-center gap-3 p-3 border border-slate-200 rounded-lg hover:bg-slate-50 transition-colors bg-white relative group/att">
                            <div className={`w-10 h-10 rounded flex items-center justify-center text-white ${att.type === 'link' ? 'bg-blue-500' : 'bg-red-500'}`}>{att.type === 'link' ? <LinkIcon size={18} /> : <FileText size={18} />}</div>
                      ...
                      <div className="flex-1 relative">
                        ...
                        <div className="flex items-center gap-2 mt-2 justify-end">
                          ...
                          <div className="relative" ref={emojiRef}>
                            <button onClick={() => setIsEmojiOpen(!isEmojiOpen)} className="p-2 text-slate-500 hover:bg-slate-100 rounded-lg transition-colors"><Smile size={16} /></button>
                            {isEmojiOpen && (
                              <div className="absolute bottom-full right-0 mb-2 bg-white border border-slate-200 shadow-xl rounded-xl p-3 grid grid-cols-5 gap-2 w-64 z-50">
                                {EMOJI_LIST.map(e => (
                                  <button key={e} onClick={() => setCommentInput(prev => prev + e)} className="text-2xl hover:bg-slate-100 p-1 rounded transition">{e}</button>
                                ))}
                              </div>
                            )}
                          </div>
                          {/* Allow multiple files */}
                          <input type="file" className="hidden" ref={chatFileRef} multiple onChange={handleCommentFileChange} />
                      ...
                      {comments.map(c => (
                        <div key={c.id} className="group w-full mb-4">
                          <div className="flex items-baseline gap-2 mb-1">
                            ...
                            {c.isEdited && <span className="text-[10px] text-slate-400">(edited)</span>}
                          </div>
