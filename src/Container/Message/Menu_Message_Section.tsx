...
import { 
  ...
  FileText, Inbox, Filter, ArrowLeft, Loader2, X, Calendar
} from 'lucide-react';
...
export default function Menu_Message_Section() {
  ...
  const handleSelectMessage = (id: number) => {
    setSelectedId(id);
    // TODO: Add logic to mark as read if needed
  };
        ...
        <div className="flex gap-2 w-full md:w-auto flex-col md:flex-row items-center">
            ...
            <div className="relative w-full md:w-auto">
                <div className="relative">
                    ...
                    <Calendar className="absolute right-2 top-1/2 -translate-y-1/2 text-gray-400 pointer-events-none" size={16} />
                </div>
            </div>
            ...
            <div className="flex bg-white rounded-xl border border-gray-200 p-1 w-full md:w-auto shrink-0">
                <button 
                    ...
                    className={`flex-1 md:flex-none px-3 py-1.5 rounded-lg text-xs font-medium transition-all ${statusFilter === '' ? 'bg-blue-100 text-blue-700' : 'text-gray-500 hover:bg-gray-50'}`}
                >
                    ทั้งหมด
                </button>
                <button 
                    onClick={() => setStatusFilter('new')} 
                    className={`flex-1 md:flex-none px-3 py-1.5 rounded-lg text-xs font-medium transition-all ${statusFilter === 'new' ? 'bg-blue-100 text-blue-700' : 'text-gray-500 hover:bg-gray-50'}`}
                >
                    ใหม่
                </button>
            </div>
            ...
            <div className="flex-1 overflow-y-auto relative">
                ...
                {!loading && messages.length === 0 && (
                    <div className="p-8 text-center text-gray-400 flex flex-col items-center">
                        ...
                        {filterDate && <p className="text-xs text-gray-300">ลองเปลี่ยนวันที่ค้นหา หรือล้างตัวกรอง</p>}
                    </div>
                )}
                ...
                {messages.map((msg) => (
                    <div 
                        ...
                        className={`p-4 border-b border-gray-50 cursor-pointer transition-all hover:bg-blue-50 group relative
                            ...
                            ${msg.status === 'new' ? 'bg-white' : 'bg-gray-50/50'}
                        `}
                    >
                        <div className="flex justify-between items-start mb-1">
                            ...
                            <span className={`text-[10px] whitespace-nowrap ${msg.status === 'new' ? 'text-blue-600 font-bold' : 'text-gray-400'}`}>
                                {formatDate(msg.createdAt)}
                            </span>
                        </div>
                        <p className={`text-sm mb-1 truncate ${msg.status === 'new' ? 'text-gray-800 font-semibold' : 'text-gray-600'}`}>
                            {msg.subject}
                        </p>
                ...
                <>
                    ...
                    <div className="p-4 border-b border-gray-100 flex justify-between items-center bg-white sticky top-0 z-10">
                        <div className="flex gap-2 items-center">
                            ...
                            <button className="p-2 hover:bg-red-50 text-gray-500 hover:text-red-600 rounded-lg" title="Delete">
                                <Trash2 size={18}/>
                            </button>
                        </div>
                        <div className="flex gap-2">
                            ...
                            <span className={`px-3 py-1 text-xs rounded-full font-medium ${selectedMessage.status === 'new' ? 'bg-blue-100 text-blue-700' : 'bg-gray-100 text-gray-600'}`}>
                        ...
                        <div className="flex justify-between items-start mb-6">
                                ...
                                <div className="flex items-center gap-3">
                                    <div className="w-10 h-10 rounded-full bg-blue-600 flex items-center justify-center text-white font-bold text-lg shrink-0 uppercase">
                                        {(selectedMessage.name || selectedMessage.email || '?').charAt(0)}
                                    </div>
                                    <div>
                    ...
                    <div className="p-4 border-t border-gray-100 bg-gray-50 sticky bottom-0">
                        <button className="w-full py-3 border border-gray-300 rounded-lg text-gray-500 text-sm text-left px-4 hover:border-blue-400 hover:bg-white transition-all flex items-center gap-2">
                            <Reply size={16}/> ตอบกลับ {selectedMessage.name || 'ผู้ติดต่อ'}...
                        </button>
                    </div>
                </>
            ) : (
