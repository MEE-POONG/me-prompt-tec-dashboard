...
import { 
  ...
  Calendar, Clock, User, CheckCircle2, File 
} from "lucide-react";
...
export default function ProjectReport() {
  ...
  useEffect(() => {
    const handleClickOutside = (event: MouseEvent) => {
      ...
      if (metricRef.current && !metricRef.current.contains(event.target as Node)) setIsMetricOpen(false);
    };
  ...
  // Mock Data (Dynamic based on state)
  const stats = {
    totalTasks: reportType === "Planning" ? 3 : 5,
    totalTime: reportType === "Planning" ? "0 mins" : "120 mins",
    totalMembers: 4
  };
    ...
    <div className="flex flex-col h-full bg-slate-50 relative overflow-hidden font-sans text-slate-800">
      ...
      <div className="px-6 py-4 bg-white border-b border-gray-200 flex flex-wrap items-center justify-between gap-4 shrink-0 z-20">
        ...
        <div className="flex items-center gap-4">
            ...
            <div className="relative" ref={typeRef}>
                ...
                >
                    {reportType === "Planning" ? <FileText size={16} className="text-gray-500"/> : <TrendingUp size={16} className="text-gray-500"/>}
                    {reportType} Report
                ...
                {isRangeOpen && (
                    <div className="absolute top-full right-0 mt-2 w-56 bg-white rounded-xl shadow-xl border border-gray-100 py-1 animate-in fade-in zoom-in-95">
                        {["This Week", "Last Week", "This Month"].map((range) => (
                            <button 
                                key={range}
                                onClick={() => { setTimeRange(range as TimeRange); setIsRangeOpen(false); }}
                                className={`w-full text-left px-4 py-2.5 text-sm font-medium hover:bg-slate-50 flex items-center justify-between ${timeRange === range ? "text-blue-600 bg-blue-50" : "text-gray-700"}`}
                            >
                                {range}
                ...
                {isMetricOpen && (
                    <div className="absolute top-full right-0 mt-2 w-48 bg-white rounded-xl shadow-xl border border-gray-100 py-1 animate-in fade-in zoom-in-95">
                        {["Estimate Time", "Actual Time"].map((metric) => (
                            <button 
                                key={metric}
                                onClick={() => { setTimeMetric(metric as TimeMetric); setIsMetricOpen(false); }}
                                className={`w-full text-left px-4 py-2.5 text-sm font-medium hover:bg-slate-50 ${timeMetric === metric ? "text-blue-600 bg-blue-50" : "text-gray-700"}`}
                            >
                                {metric}
                            </button>
                        ))}
      ...
      <div className="flex-1 overflow-y-auto custom-scrollbar p-6 space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              ...
              <div className="bg-white p-6 rounded-2xl border border-gray-100 shadow-sm flex flex-col items-center justify-center hover:border-blue-200 transition-colors">
                  <h3 className="text-gray-400 text-xs font-bold uppercase tracking-wider mb-2">Total {timeMetric}</h3>
                  ...
                  <table className="w-full text-sm text-left">
                      <thead className="text-xs text-gray-400 uppercase bg-white border-b border-gray-100">
                          <tr>
                              ...
                              <th className="px-6 py-3 font-bold text-right">{timeMetric}</th>
                          </tr>
                      </thead>
                          ...
                          {tasks.map((task) => (
                              <tr key={task.id} className="hover:bg-gray-50 transition-colors cursor-pointer group">
                                  <td className="px-6 py-4 font-bold text-gray-800 flex items-center gap-3">
                                      {task.status === "Done" ? <CheckCircle2 size={18} className="text-green-500"/> : <div className="w-4 h-4 rounded-full border-2 border-gray-300 group-hover:border-blue-400 transition-colors"></div>}
                                      {task.name}
                                  </td>
