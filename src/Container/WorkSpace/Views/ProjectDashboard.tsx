...
export default function ProjectDashboard({
  ...
  useEffect(() => {
    ...
    const fetch = async () => {
      try {
        ...
        const tasksFromCols = cols.flatMap((c: any) => (c.tasks || []).map((t: any) => ({ ...t, columnTitle: c.title })));
        if (!cancelled) {
    ...
    const url = `/api/realtime/stream?channel=${encodeURIComponent(String(boardId))}`;
  ...
  const totalTasks = localColumns.length > 0 ? localColumns.reduce((acc, c) => acc + ((c.tasks && c.tasks.length) || 0), 0) : localTasks.length;

  // Normalize status values so various sources (columns, strings) map to the same canonical states
  const normalizeStatus = (t: any) => {
    const s = (t.status || t.column || t.columnTitle || "").toString().toLowerCase();
  ...
  const countTasksInColumnsWithStatus = (statusKey: string) => {
    if (localColumns.length === 0) return localTasks.filter((t: any) => normalizeStatus(t) === statusKey).length;
    return localColumns.reduce((acc: number, col: any) => {
      if (normalizeStatus({ status: col.title }) === statusKey) return acc + ((col.tasks && col.tasks.length) || 0);
      return acc;
    }, 0);
  };
  ...
  const now = new Date();
  const overdueTasks = localTasks.filter(t => t.rawDueDate && new Date(t.rawDueDate) < now && normalizeStatus(t) !== "completed").length;
  ...
  const completedCountsByDay = last7Days.map((day) => {
    ...
    return localTasks.filter((t: any) => t.completedAt && new Date(t.completedAt).getTime() >= start && new Date(t.completedAt).getTime() < end).length;
  });

  const maxCount = Math.max(...completedCountsByDay, 1);
  const chartData = completedCountsByDay.map(c => Math.round((c / maxCount) * 100));
  const days = last7Days.map(d => d.toLocaleDateString(undefined, { weekday: 'short' }));
