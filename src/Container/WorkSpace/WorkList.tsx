...
// --- Mock Data ---
// วันที่ใน mock data ต้องเป็น YYYY-MM-DD เพื่อให้ตรงกับปฏิทิน
const myProjectsData: Project[] = [
  {
    ...
    members: 5,
    updated: "2h ago",
    createdAt: "2025-12-10", // ตรงกับเดือน Dec 2025
...
export default function WorkList({
  ...
  // Calendar State: เริ่มต้นที่ Dec 2025 ตาม Mock Data
  const [selectedDate, setSelectedDate] = useState<string | null>(null); 
  const [currentCalendarDate, setCurrentCalendarDate] = useState(new Date(2025, 11, 1)); 
  ...
  // Combine all projects for calendar markers
         ...
         <div className="bg-white p-5 rounded-2xl shadow-sm border border-gray-200">
            ...
            <div className="grid grid-cols-7 text-center gap-y-3 text-sm text-gray-700">
               ...
               {[...Array(new Date(currentCalendarDate.getFullYear(), currentCalendarDate.getMonth() + 1, 0).getDate())].map((_, i) => {
                   const day = i + 1;
                   const dateStr = `2025-${(currentCalendarDate.getMonth() + 1).toString().padStart(2, '0')}-${day.toString().padStart(2, '0')}`;
                   const isSelected = selectedDate === dateStr;
                   
                   // ✅ เช็คว่าวันนี้มีโปรเจกต์หรือไม่
                   const hasEvent = allProjects.some(p => p.createdAt === dateStr);
                       ...
                       <div key={day} className="relative flex justify-center flex-col items-center">
                           <button 
                               ...
                               className={`h-7 w-7 rounded-full flex items-center justify-center transition-all ${
                                   ...
                                   // ✅ ถ้ามีโปรเจกต์แต่ไม่ได้เลือก ให้เป็นตัวหนาสีน้ำเงิน
                                   hasEvent && !isSelected ? "font-bold text-blue-600" : ""
                               }`}
                           >
                               {day}
                           </button>
                           {/* ✅ แสดงจุดสีฟ้าใต้วันที่มีโปรเจกต์ */}
